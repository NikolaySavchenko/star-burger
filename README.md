...
Определите переменные окружения `SECRET_KEY` и `YANDEX_API_KEY`.
Где взять `YANDEX_API_KEY` можно почитать [здесь](https://dvmn.org/encyclopedia/api-docs/yandex-geocoder-api/)
